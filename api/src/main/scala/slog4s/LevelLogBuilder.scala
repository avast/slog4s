...
trait LevelLogBuilder[F[_]] {
  ...
  /** Log simple message with an exception and no message specific structured
    ...
    * @return
    */
  def apply(ex: Throwable, msg: => String)(implicit
      location: Location
  ): F[Unit] =
  ...
  /** Extends current logging statement with structured data.
    ...
    *   typeclass.
    * @return
    *   a new [[ArgLogBuilder]] instance containing provided structured argument
    */
  def withArg[T: LogEncoder](key: String, value: => T): ArgLogBuilder[F] =
    new DeferredArgLogBuilder[F](whenEnabled.withArg(key, value))

  /** Gets an instance of [[WhenEnabledLogBuilder]] associated with current
    * logging level.
    * @return
    *   an instance of [[WhenEnabledLogBuilder]]
    */
